    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float2 node_6631 = (o.uv0+(1.0 - (_Firepanspeed*node_1121.g))*float2(0,1));
                float4 _FireNoise_var = tex2Dlod(_FireNoise,float4(TRANSFORM_TEX(node_6631, _FireNoise),0.0,0));
                float node_604 = saturate((_FireNoise_var.r*(o.uv0.g*_Firedissolvepower)));
                float4 _FireMask_var = tex2Dlod(_FireMask,float4(TRANSFORM_TEX(o.uv0, _FireMask),0.0,0));
                float _IsMask_var = lerp( node_604, saturate((1.0-(1.0-((1.0 - _FireMask_var.r)*_Firesize))*(1.0-node_604))), _IsMask );
                float node_9949 = (1.0 - _IsMask_var);
                v.vertex.xyz += (node_9949*v.normal*_Displacement);
